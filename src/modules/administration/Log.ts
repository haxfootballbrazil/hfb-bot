...
class Log extends Module {
    ...
    private msgIntervalTimeMs = 7000;

    private readonly ipServerURL = process.env.UNSAFE_CONNECTION_LOG;
    private readonly connHashURL = process.env.SAFE_CONNECTION_LOG;
    private readonly chatLogURL = process.env.CHAT_LOG;

    private sendEmbedWebhook(url: string, embed: any) {
        return fetch(url, {
            method: 'post',
            headers: {
    ...
    private sendContentWebhook(url: string, content: string) {
        return fetch(url, {
            method: 'post',
            headers: {
                'Content-Type': 'application/json',
